...
pub fn build() {
    ...
    // Link Valhalla and its requirements
    ...
    // Link required system libraries (from libvalhalla.pc Requires.private)
    println!("cargo:rustc-link-lib=dylib=z");
    println!("cargo:rustc-link-lib=dylib=m");

    // Try to find protobuf-lite via pkg-config, fallback to protobuf
